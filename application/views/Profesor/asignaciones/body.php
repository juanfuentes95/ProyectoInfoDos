		...
		<tbody>
		  	<tr>
				...
				foreach ($ape->result() as $value) {
					...
					$id_curso=null;
					$arreglo['nombre_asignatura']=$this->Profesor_Model->consultar_nombre_asignatura($value->asignatura);
					foreach ($arreglo['nombre_asignatura']->result() as $value1) {
						$nombre_asignatura=$value1->nombre_asignatura;
					}

					$arreglo['nombre_profesor']=$this->Profesor_Model->consultar_nombre_profesor($value->profesor);
					foreach ($arreglo['nombre_profesor']->result() as $value2) {
						$nombre_profesor=$value2->nombre_profesor;
					}

					$arreglo['edicion']=$this->Profesor_Model->consultar_edicion_completo($value->edicion);
					foreach ($arreglo['edicion']->result() as $value3) {
						$id_curso=$value3->id_curso;
						$anio_curso=$value3->anio_curso;
					}

					$arreglo['curso']=$this->Profesor_Model->consultar_nombre_curso($id_curso);
